...
void createBox(irr::scene::ISceneManager* sceneManager, irr::video::IVideoDriver *driver,btVector3 TPosition, vector3df TPosition_irr, vector3df TScale, int texture, bool rotate){
	...
	if(rotate){
		...
		tr.setIdentity();
		btQuaternion quat;
		quat.setRotation(btVector3(1,0,0), btScalar(45*PI/180));
		tr.setRotation(quat);
...
void createObj(irr::scene::ISceneManager* sceneManager, irr::video::IVideoDriver *driver,btVector3 TPosition, vector3df TPosition_irr, vector3df TScale, int texture, bool rotate){

	btScalar TMass = 0;

	// Create OBJECT
	scene::IAnimatedMeshSceneNode* Node =  sceneManager->addAnimatedMeshSceneNode(sceneManager->getMesh("./../assets/modelos/pocion.obj"));
	Node->setMaterialFlag(video::EMF_LIGHTING, false);
...
void createScenery(irr::scene::ISceneManager* sceneManager, irr::video::IVideoDriver *driver){
	...
	TPosition = btVector3(5,4,7);
	TPosition_irr = vector3df(5,4,7);
