...
std::list<Connection*> *Pathfinding::makeAStar(NavMesh navmesh, vector3df from, vector3df to){
    std::list<Connection*> *path = NULL;
    std::vector<Node*> *nearests = navmesh.searchNearestNodes(from);//RETURNS THE FIRST ELEMENT OF THE ARRAY
    std::vector<Node*> *furthest = navmesh.searchNearestNodes(to);
    if(nearests !=NULL && furthest != NULL){
        Node* fromnode = new Node(0,from);

        //TODO:: edit the costs
        Connection start1(0.5F, fromnode, nearests->at(0));
        Connection start2(0.5F, fromnode, nearests->at(1));
        Connection start3(0.5F, fromnode, nearests->at(2));

        Node* tonode = new Node(0,to);
        Connection end1(0.5F, furthest->at(0), tonode);
        Connection end2(0.5F, furthest->at(1), tonode);
        Connection end3(0.5F, furthest->at(2), tonode);

        path =  AStar(fromnode,tonode);
...
std::list<Connection*> *Pathfinding::AStar( Node* StartNode , Node* EndNode){
